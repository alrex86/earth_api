...
import { userDBType, userSessionsType } from "../types"
...
const Users = {
    ...
    createUser: (username: string, password: string) => {
        ...
        return new Promise((resolve, reject) => {
            db.dbData.connection.query("INSERT INTO " + Users.dbName + " set ?", user, (err, res) => {
                if(err) {
    ...
    getUserByUsername: (username: string): Promise<userDBType> => {
        return new Promise((resolve, reject) => {
            db.dbData.connection.query("SELECT * FROM " + Users.dbName + " WHERE username = ?", [username], (err, res) => {
                if(err) {
                    console.log("error: ", err);
                    // result(err, null);
                }else{
    ...
    getUserByUserID: (id: number): Promise<userDBType> => {
        return new Promise((resolve, reject) => {
            db.dbData.connection.query("SELECT * FROM " + Users.dbName + " WHERE id = ?", [id], (err, res) => {
                if(err) {
                    console.log("error: ", err);
                    // result(err, null);
                }else{
