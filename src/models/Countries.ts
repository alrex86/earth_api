...
const Countries = {
    ...
    createCountry: (newCountry: countryDBType): Promise<number> => {
        ...
        return new Promise((resolve, reject) => {
            db.dbData.connection.query("INSERT INTO " + Countries.dbName + " set ?", newCountry, (err, res) => {
                if(err) {
    ...
    getCountryByUserID: (userID: number): Promise<countryDBType> => {
        ...
        // let query = "INSERT INTO users set ?", newUser, (err, res) => {}
        return new Promise((resolve, reject) => {
            db.dbData.connection.query("SELECT * FROM " + Countries.dbName + " WHERE userid = ?", [userID], (err, res) => {
                if(err) {
                    console.log("error: ", err);
                    resolve(null);
                    // result(err, null);
    ...
    getCountryByID: (countryID: number): Promise<countryDBType> => {
        ...
        // let query = "INSERT INTO users set ?", newUser, (err, res) => {}
        return new Promise((resolve, reject) => {
            db.dbData.connection.query("SELECT * FROM " + Countries.dbName + " WHERE id = ?", [countryID], (err, res) => {
                if(err) {
                    console.log("error: ", err);
                    resolve(null);
                    // result(err, null);
    ...
    build: (amtFarm: number, amtBusiness: number, amtResearch: number, countryID: number): buildResultType => {
        ...
        while(amtFarm != 0 || amtBusiness != 0){
            ...
            country.allBuildings.farm.amt = country.allBuildings.farm.amt + amtToBuildFarm; 
            buildingsBuilt.farm = buildingsBuilt.farm + amtToBuildFarm;
            country.allBuildings.business.amt = country.allBuildings.business.amt + amtToBuildBusiness; 
    ...
    research: (amtFarm: number, amtBusiness: number, countryID: number): researchResultType => {
        ...
        while(amtFarm != 0 || amtBusiness != 0){
            ...
            country.allBuildings.farm.amt = country.allBuildings.farm.amt + amtToResearchFarm; 
            techsResearched.farm = techsResearched.farm + amtToResearchFarm;
            country.allBuildings.business.amt = country.allBuildings.business.amt + amtToResearchBusiness; 
            techsResearched.business = techsResearched.business + amtToResearchBusiness;
            // country.allBuildings.researchLab.amt = country.allBuildings.researchLab.amt + amtResearch;
