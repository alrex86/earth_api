...
const Tokens = {
    ...
    createToken: (token: string, userID: number): Promise<number> => {
        ...
        return new Promise((resolve, reject) => {
            db.dbData.connection.query("INSERT INTO " + Tokens.dbName + " set ?", newToken, (err, res) => {
                if(err) {
    ...
    getTokenByUserID: (userID: number): Promise<newTokenType> => {
        return new Promise((resolve, reject) => {
            db.dbData.connection.query("SELECT * FROM " + Tokens.dbName + " WHERE userid = ?", [userID], (err, res) => {
                if(err) {
                    console.log("error: ", err);
                    // result(err, null);
                }else{
    ...
    getTokenByTokenID: (tokenID: number): Promise<newTokenType> => {
        return new Promise((resolve, reject) => {
            db.dbData.connection.query("SELECT * FROM " + Tokens.dbName + " WHERE id = ?", [tokenID], (err, res) => {
                if(err) {
                    console.log("error: ", err);
                    // result(err, null);
                }else{
